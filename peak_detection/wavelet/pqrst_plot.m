function pqrst_plot(ECG_signal, fs, time_vector, duration, ecg_orig)
    ...
    QRS_interval = round(0.095 * fs * beatPeriod); % 82 ms
    P_interval = round(0.2 * fs * beatPeriod);  % 220 ms
    ...
    % Plot R peaks
    scatter((R_loc), ECG_signal(R_loc), 'ko', 'filled', 'DisplayName', 'R Peaks');

    % Plot P point
    valid_P_points = P_points(~isnan(P_points));
    scatter((valid_P_points), ECG_signal(valid_P_points), 'ro', 'filled', 'DisplayName', 'P Points');
    ...
    scatter((valid_Q_points), ECG_signal(valid_Q_points), 'go', 'filled', 'DisplayName', 'Q Points');
    scatter((valid_S_points), ECG_signal(valid_S_points), 'bo', 'filled', 'DisplayName', 'S Points');
    scatter((valid_T_points), ECG_signal(valid_T_points), 'co', 'filled', 'DisplayName', 'T Points');
