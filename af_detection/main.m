...
data_path = fullfile('data/mimic_raw/AF/252/00003/', matFileName);

data = load(data_path);

invalid_p_count = 0;  % Initialize the count of invalid P-points
...
y = abs(y) ;
avg = mean(y);
[Rpeaks, locs] = findpeaks(y, t, 'MinPeakHeight', 7 * avg, 'MinPeakDistance', 50);
nohb = length(locs);
...
search_window_size = 20;  % Number of samples before and after Ri
...
for i = 1:nohb
    ...
    q_window_start = max(1, round(accurate_R_positions(i)) - round(qrs_window_size / 2));
     q_window_end = (round(accurate_R_positions(i) )-1);
    s_window_end = min(length(ecg), accurate_R_positions(i) + round(qrs_window_size / 2));
    s_window_start = (round(accurate_R_positions(i) )+1);
...
for i = 1:nohb
    % Determine the amplitude of the P-peak
    p_amplitude = y1(p_wave_positions(i));
    
    % Determine the amplitude of the R-peak
    r_amplitude = Rpeaks(i);
    
    % Check if the P-peak is invalid based on the amplitude criteria
    if p_amplitude < 0.1 * r_amplitude
        invalid_p_count = invalid_p_count + 1;
    end
end
disp(['Invalid P-points count: ' num2str(invalid_p_count)]);
