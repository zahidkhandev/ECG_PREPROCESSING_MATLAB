function time_pqrst_plot_af_detection(ECG_signal, fs, time_vector, duration)
    ...
    fprintf('\nInvalid P Point Count: %d', invalid_P_Count);
    fprintf('\nValid R Point Count: %d', length(R_loc));

    if invalid_P_Count < 0.75 * length(R_loc)
        fprintf('\nIt is an AF signal');
    else
        fprintf('\nIt is not an AF signal');
    end
